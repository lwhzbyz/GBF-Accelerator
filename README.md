...
登录自启动可建立指向固定安装路径的快捷方式，参数使用 `--gui --autostart --start-minimized --data-dir "C:\GBF_data"`。这些参数会启动代理并隐藏到托盘；启动失败时保留界面。普通双击仍不自动启用代理。是否设置系统 PAC 取决于已保存的配置，参数本身不会安装证书或修改浏览器设置。
